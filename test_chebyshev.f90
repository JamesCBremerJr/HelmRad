...
double precision, allocatable :: ab(:,:),vals0(:,:),coefs0(:,:),ts(:),vals(:),vals1(:)
double precision, allocatable :: z(:),w(:,:),coefs(:),roots(:)
...
! allocate(coefs0(k,nints))
! do int=1,nints
! coefs0(:,int) = matmul(chebdata%u,vals0(:,int))
! end do


! call chebpw_roots(ier,k,nints,ab,coefs0,nroots,roots)
! call prin2("roots = ",roots)
! call prin2("vals at roots = ",cos(133*roots))
