...
program expr_volcano
use utils
use chebyshev
use helmrad
use helmop
use expr_volcano_functions
...
double precision, allocatable   :: xsings(:),xs0(:),ys0(:),xsave(:,:),amatr(:,:)
...
call potfun(1,l+1,rs,qs,userptr)
...
call potfun(1,nn,xs0,ys0,userptr)


iw = 101
open(iw,FILE='volcano_q.py')
call pyplot_begin(iw,istatus)

call pyplot_add_function(istatus,1,"",nn,xs0,ys0)
call pyplot_ylabel(istatus,"q(r)")
call pyplot_xlabel(istatus,"r")


call pyplot_end(istatus,"volcano_q.pdf")
call system("python volcano_q.py")
...
call image_incident(ipart,"volcano_incident.pdf",2*R,dlambda,wavefun,userptr)
call helmrad_init(ifout,eps,m,xsings,R,dlambda,potfun,userptr,helmdata)
call helmrad_solve(helmdata,wavefun,userptr,scatdata)
call image_output(helmdata,scatdata,ipart,"volcano_total.pdf","volcano_scattered.pdf",&
...
iw = 20
open(iw,FILE='volcano.dat',STATUS='old')
...
iw = 25
open(iw,FILE='volcano_table.tex')
...
open(iw,FILE='volcano_graph1.py')
...
call pyplot_end(istatus,"volcano_graph1.pdf")
call system("python volcano_graph1.py")





open(iw,FILE='volcano_graph2.py')
...
call pyplot_end(istatus,"volcano_graph2.pdf")
call system("python volcano_graph2.py")
