...
!
!    show_matrix - produce a PDF plot of a matrix (in the style of MATLAB's
!     "imagesc" command) using python and matplotlib
!
!  It also contains the following  more sophisticated set of plotting routines which
!  are designed to produce plots for use in publications:
!  
!
!  Table generatation:
!  ------------------
...
print *,str
print "(3(d24.15,',',d24.15,2X))",a

write (13,*) str
write (13,"(3(d24.15,',',d24.15,2X))") a
...
subroutine show_matrix(ifgray,xlabel,ylabel,x1,x2,y1,y2,filename,amatr)
implicit double precision (a-h,o-z)
...
!
!  Input parameters:
!     ifgray - an integer parameter indicating whether the plot should be in
!       grayscale (ifgray = 1) or color (ifgray = 0)
!     filename - the name of the PDF file to produce
!     xlabel -  a label for the x axis
!     ylabel - a label for the y axis
!     (x1,x2) - extents of the labels for the x-axis
!     (y1,y2) - extents of the labels for the y-axis
!     amatr - the (n,m) real-valued matrix to "show"
!
!  Output parameters:
!     NONE
!
...
write(command2,"(A,A,I5.5,A)")  "rm     ","plot",ii,".py"
...
if (ifgray == 1) then
write(iw,"('plt.imshow(vals,',A,',extent=(',E24.15,',',E24.15,',',E24.15,',',E24.15,'))')") &
  "cmap='gray'",x1,x2,y1,y2
else
write(iw,"('plt.imshow(vals,',A,',extent=(',E24.15,',',E24.15,',',E24.15,',',E24.15,'))')") &
  "cmap='jet'",x1,x2,y1,y2
endif

write(iw,"(A)") "plt.colorbar()"
...
subroutine plot_points(filename,n,xs,ys)
implicit double precision (a-h,o-z)
...
write(scriptname,"(A,I5.5,A)") "plot",ii,".py"
write(command,"(A,A,I5.5,A)")  "python ","plot",ii,".py &"
write(command,"(A,A,I5.5,A)")  "rm -f   ","plot",ii,".py &"
...
write(iw,"(A)") "mpl.rcParams.update({'font.size': 14})"
...
subroutine pyplot_xlogscale2(istatus)
implicit double precision (a-h,o-z)
...
write (iw,"(A)") "ax.set_xscale('log',basex=2)"
istatus = iw + ishft(idx,8) + ishft(ilabel,16) + ishft(ilogx,17) + ishft(ilogy,18)
...
subroutine pyplot_ylogscale2(istatus)
implicit double precision (a-h,o-z)
...
write (iw,"(A)") "ax.set_yscale('log',basey=2)"
istatus = iw + ishft(idx,8) + ishft(ilabel,16) + ishft(ilogx,17) + ishft(ilogy,18)
...
subroutine bisected_intervals2(dd,nints,a,b,ab)
implicit double precision (a-h,o-z)
...
do int=2,nints
ab(1,int) = dd**(-nints+int-1) 
ab(2,int) = dd**(-nints+int)
